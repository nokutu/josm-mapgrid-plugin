...
public class MapGridLayer extends Layer {
  ...
  public MapGridLayer(MapGridPlugin plugin) {
    ...
    plugin.markAsTodoAction.setLayer(this);
    plugin.markAsDoingAction.setLayer(this);
    plugin.markAsDoneAction.setLayer(this);
  }
  ...
  public void setSelectedGrid(Grid grid) {
    ...
    plugin.markAsTodoMenu.setEnabled(false);
    plugin.markAsDoingMenu.setEnabled(false);
    plugin.markAsDoneMenu.setEnabled(false);
    if (grid != null) {
      selectedGrid.select();
      plugin.splitGridMenu.setEnabled(true);
      plugin.markAsTodoMenu.setEnabled(true);
      plugin.markAsDoingMenu.setEnabled(true);
      plugin.markAsDoneMenu.setEnabled(true);
    }
    Main.map.mapView.repaint();
  }
  ...
  public void destroy() {
    ...
    plugin.markAsTodoAction.removeLayer();
    plugin.markAsDoingAction.removeLayer();
    plugin.markAsDoneAction.removeLayer();
  }
}
